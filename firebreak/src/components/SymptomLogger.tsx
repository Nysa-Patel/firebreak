...
interface Props {
  aqi: number | null;
  /** Which household member (or previewed persona) this check-in belongs to
   * -- "me" keeps the original bare device id for continuity with logs from
   * before household members existed; anything else gets its own bucket so
   * family members' symptom histories never mix. */
  memberId: string;
  memberName: string;
}

export function SymptomLogger({ aqi, memberId, memberName }: Props) {
  ...
  useEffect(() => {
    ...
    // Backend caps device_id at 64 chars -- slice defensively so an older,
    // longer member id (from before member ids were shortened) can't overflow it.
    const id = memberId === "me" ? deviceId : `${deviceId}:${memberId.slice(0, 10)}`;
    setEffectiveId(id);
    setThreshold(null);
  ...
  async function handleLog(severity: SymptomSeverity) {
    if (effectiveId == null || aqi == null) return;
    setStatus("logging");
    try {
      await logSymptom(effectiveId, severity, aqi);
      setLoggedToday(true);
      setStatus("idle");
      getPersonalThreshold(effectiveId)
        .then(setThreshold)
        .catch(() => {});
    } catch {
      setStatus("error");
  ...
  return (
    <div className="p-5">
      <SectionLabel>{memberName === "Me" ? "How are you feeling today?" : `How is ${memberName} feeling today?`}</SectionLabel>
      ...
      {threshold?.has_enough_data ? (
        <div className="mt-4 pt-4" style={{ borderTop: "1px solid var(--border)" }}>
          <p className="text-xs uppercase tracking-wide" style={{ color: "var(--text-muted)" }}>
            {memberName === "Me" ? "Your personal threshold" : `${memberName}'s personal threshold`}
          </p>
